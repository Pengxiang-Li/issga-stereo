...
def dataloader(filepath, typ = 'train'):
  ...
  if typ == 'train':
    # train = [image[i] for i in range(200) if i not in imglist]*4
    train = [image[i] for i in range(200)]
    train = train + [image[i] for i in range(200) if i in imglist]
  elif typ == 'trainval':
    train = [image[i] for i in range(200) if i in imglist]
