...
class projfeat3d(nn.Module):
    ...
    def forward(self,x):
        ...
        x = self.bn(x)
        print("self.bn.running_mean {}".format(self.bn.running_mean))
...
class disparityregression(nn.Module):
    ...
    def forward(self, x,ifent=False, mul = False):
...
class decoderBlock(nn.Module):
    def __init__(self, nconvs, inchannelF,channelF,stride=(1,1,1),up=False, nstride=1,pool=False,last =  False):
