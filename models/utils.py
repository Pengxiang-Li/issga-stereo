...
class unet(nn.Module):
    def __init__(self):
        ...
        # Encoder
        self.convbnrelu0_1 = conv2DBatchNormRelu(in_channels=3, k_size=3, n_filters=16,
                                                 padding=1, stride=1, bias=False)
        self.convbnrelu0_2 = conv2DBatchNormRelu(in_channels=16, k_size=3, n_filters=16,
                                                 padding=1, stride=1, bias=False)
        self.convbnrelu0_3 = conv2DBatchNormRelu(in_channels=16, k_size=3, n_filters=32,
                                                padding=1, stride=1, bias=False)
        self.convbnrelu1_1 = conv2DBatchNormRelu(in_channels=32, k_size=3, n_filters=32,
                                                 padding=1, stride=2, bias=False)
        self.convbnrelu1_2 = conv2DBatchNormRelu(in_channels=32, k_size=3, n_filters=32,
                                                 padding=1, stride=1, bias=False)
        # Vanilla Residual Blocks
        self.res_block2 = self._make_layer(residualBlock,32,2,stride=1)
        ...
        self.upconv3 = nn.Sequential(nn.Upsample(scale_factor=2),
                                     conv2DBatchNormRelu(in_channels=16, k_size=3, n_filters=16,
                                                 padding=1, stride=1, bias=False))
        self.iconv3 = conv2DBatchNormRelu(in_channels=128, k_size=3, n_filters=64,
                                                 padding=1, stride=1, bias=False)
        self.upconv2 = nn.Sequential(nn.Upsample(scale_factor=4),
                                     conv2DBatchNormRelu(in_channels=32, k_size=3, n_filters=16,
                                                 padding=1, stride=1, bias=False))
        ...
        self.proj2 = conv2DBatchNormRelu(in_channels=32, k_size=1,n_filters=16, padding=0,stride=1,bias=False)
        self.proj1 = conv2DBatchNormRelu(in_channels=32, k_size=1,n_filters=16, padding=0,stride=1,bias=False)
    ...
    def forward(self, x):
        # H, W -> H/2, W/2
        conv0 = self.convbnrelu0_1(x)
        conv0 = self.convbnrelu0_2(conv0)
        conv0 = self.convbnrelu0_3(conv0)

        conv1 = self.convbnrelu1_1(conv0)
        ...
        proj1 = self.proj1(conv0)

    
        
        return proj6,proj5,proj4,proj3,proj2,proj1  #  1/64 1/32  1/16  1/8  1/4  1
